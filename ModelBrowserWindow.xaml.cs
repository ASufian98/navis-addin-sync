...
{
    /// <summary>
    /// Browses a project — area, folder, model, version — and saves the chosen version.
    /// </summary>
    public partial class ModelBrowserWindow : Window
    {
        ...
        private async System.Threading.Tasks.Task LoadVersionsAsync(BimDesign design)
        {
            try
            {
                ...
                var versions = await _api.GetVersionsByDesignAsync(_projectId, design.DesignId, area);
        ...
        private void VersionsListBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (!model.Source.IsDownloadable)
            {
                ...
                ShowError("Your role can browse this model but not download it.");
                return;
            }
        ...
        private void ChangeDestinationButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            var dialog = new Microsoft.Win32.SaveFileDialog
            {
                ...
                DefaultExt = Path.GetExtension(current),
                Filter = "All files (*.*)|*.*",
        ...
        {
            var folder = FoldersListBox.SelectedItem as FolderRow;
            string folderName = folder != null ? SafeName(folder.Name) : "folder";

            string fileName = model.Name;
            if (string.IsNullOrWhiteSpace(fileName)) fileName = "model";
            ...
            if (string.IsNullOrEmpty(ext)) ext = ".nwd";
